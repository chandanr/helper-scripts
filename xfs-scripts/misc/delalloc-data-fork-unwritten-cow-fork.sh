...
echo "Create unwritten extent in cow fork"
if (( 0 )); then
	perf record \
	     -e xfs:\* -g -a -- xfs_io -c "sync_range 0 4k" $file1 &>/dev/null
else
	xfs_io -c "sync_range 0 4k" $file1 &>/dev/null
fi
...
perf record \
     ...
     -e probe:xfs_bmapi_convert_delalloc \
     -e probe:xfs_bmapi_convert_delalloc_L56 \
